    ...
    namespace Base {
        ...
        public:
            ...
            */
             ...
             *                  implement atmospheric effects like the \ref SilverLiningPlugin are
             *                  expecting this to be only in the range of 0.0-1.0
             * \author    Trajce Nikolov Nick openig@compro.net
             * \copyright (c)Compro Computer Services, Inc.
             * \date      Sun Jan 11 2015
             */
            ...
            /*! Enables a cloud layer already loaded into the atmosphere. The \ref openig::OpenIG is not implementing this instead it uses
             *  \ref OpenIG::PluginBase::PluginContext::Attribute to pass commands to all the plugins that might deal with
             *  such atmospheric effects. See \ref addCloudLayer for more info
             * \brief     Enables a previously loaded but distabled cloud layer in the scene
             * \param id     The id of the clouds layer file to be enabled in the scene
             * \param enable Whether we are enabling or disabling this cloudLayer
            ...
            /*! Adds a new cloud layer file to the system. The \ref openig::OpenIG is not implementing this feature instead
             ...
             *                  implement atmospheric effects like the \ref SilverLiningPlugin are
             *                  expecting this to be only in the range of 0.0-1.0
             * \param enable    The enable setting for the cloudlayer, enabled(true) you can see it,
             *                  disabled(false) it can be loaded but not visible and also not updated
             *                  to increase performance.  Enable it and its seen immediately with no delay/pause.
             * \param filename  The full path where the saved cloud file will be written
             * \author    Curtis Rubel openig@compro.net
             * \copyright (c)Compro Computer Services, Inc.
             * \date      Tue Nov 8 2016
             */
            virtual void createCloudLayerFile(unsigned int id, int type, double altitude, double thickness=0, double density=0.3, bool enable=true, const std::string& filename="") = 0;

            /*! Removes a cloud layer file from the system. The \ref openig::OpenIG is not implementing this instead it uses
             *  \ref OpenIG::PluginBase::PluginContext::Attribute to pass commands to all the plugins that might deal with
             *  such atmospheric effects. See \ref addCloudLayer for more info
             * \brief Removes a cloud layer file from the system
             * \param id        The id of the clouds layer file to be removed, to ensure its also removed from the scene
             * \param filename  The full path to the cloud file to be deleted
             * \author    Curtis Rubel openig@compro.net
             * \copyright (c)Compro Computer Services, Inc.
             * \date      Tue Nov 08 2016
             */
             ...
             *  \ref igplugincore::PluginContext::Attribute to pass commands to all the plugins that might deal with
             *  such atmospheric effects. See \ref loadCloudLayer for more info
             * \param id        The id of the cloud layer
             ...
             */
            virtual void loadCloudLayerFile(unsigned int id, int type, const std::string& filename) = 0;
            ...
            */
             ...
             * \param hard		true for enable/disable even if this is animated light wich might
             *					internally use callback for simulating pulses via this call
             * \author    Trajce Nikolov Nick openig@compro.net
             * \copyright (c)Compro Computer Services, Inc.
