...
void overrideBillboardFragment_forward_plus_sl_ps(in vec4 texColor, in vec4 lightColor, inout vec4 finalColor)
{
   ...
   float factor = min(LightBrightnessOnClouds*texColor.a,0.5);
   vec4 lc = vec4(lightColor.r,lightColor.g,lightColor.b,0.0);
   finalColor = texColor*(vec4(lc.xyz,texColor.a)+vec4(fAmbientColor.xyz+fDiffuseColor.xyz+fSpecularColor.xyz,0.0)*factor);
