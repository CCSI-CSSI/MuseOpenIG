...
#include <Library-Graphics/GPGPUDevice.h>
...
void LightManagerStateAttribute::set(OpenIG::Library::Graphics::LightManager*	 lightManager
    ...
    , OpenIG::Library::Graphics::GPGPUDevice* gpgpudevice
    , bool bUseMultipleCPUCores
    , osg::Group* scene
    ...
    )
{
    ...
    _gpgpudevice = gpgpudevice;

    _lightData = new LightData(341, FORMAT_R32G32B32A32_FLOAT);
    Vector2_uint32 tileSize(32, 32);
    _tileSpaceLightGrid = new TileSpaceLightGrid(tileSize, _gpgpudevice, bUseMultipleCPUCores);
...
void LightManagerStateAttribute::updateLightDataTBO(void)
{
    if (_lightDataTBO==0)
    {
        _lightDataTBO = new osg::TBO(_lightData->GetWidth(), _lightData->GetFormat(), _extensions, _gpgpudevice);
    }
...
void LightManagerStateAttribute::resizeTileLightGridOffsetAndSizeTBOIfNeeded(size_t numLights, const Vector2_uint32& viewPortSize, const Vector2_uint32& tileSize)
{
    size_t szRequired = TileSpaceLightGrid::GetTileGridOffsetAndSizeWidth(viewPortSize, tileSize);
    if (_lightGridOffsetAndSizeTBO==0)
    {
        _lightGridOffsetAndSizeTBO = new osg::TBO(szRequired
            , _tileSpaceLightGrid->GetTileGridOffsetAndSizeDataFormat(), _extensions, _gpgpudevice);
    }
    ...
    if (_lightGridOffsetAndSizeTBO->getWidth()!=szRequired)
    {
        _lightGridOffsetAndSizeTBO->resize(szRequired);
    }
    if (_lightGridOffsetAndSizeTBO->isValid()==false)
    {
        std::cout<<"Error"<<std::endl;
...
void LightManagerStateAttribute::resizeTileLightIndexListTBOIfNeeded(size_t numLights, const Vector2_uint32& viewPortSize, const Vector2_uint32& tileSize)
{
    unsigned int widthRequired = Math::GetUpperPowerOfTwo(TileSpaceLightGrid::GetTileLightIndexListLengthUpperBound(numLights, viewPortSize, tileSize)/4+1)*4;
    if (_lightIndexListTBO==0)
    {
        _lightIndexListTBO = new osg::TBO(widthRequired
            , FORMAT_R32G32B32A32_SINT, _extensions, _gpgpudevice);
    }
    else
    {
...
{
    const VectorLights& frustumAffectingLights = _lightManager->GetFrustumAffectingLights();

    size_t numLights = frustumAffectingLights.size();

    resizeTileLightGridOffsetAndSizeTBOIfNeeded(numLights, *_fpViewport, _tileSpaceLightGrid->GetTileSize());
    resizeTileLightIndexListTBOIfNeeded(numLights, *_fpViewport, _tileSpaceLightGrid->GetTileSize());

    // PPP: Clean this up
    if (_gpgpudevice)
    {
        int lightGridOffsetAndSizeBufferID = _lightGridOffsetAndSizeTBO->bufferID();
        int lightIndexListBufferID         = _lightIndexListTBO->bufferID();

        _tileSpaceLightGrid->Update(_lightManager->GetFrustumAffectingLights(), _fpCamera, *_fpViewport
            , &lightGridOffsetAndSizeBufferID, _lightGridOffsetAndSizeTBO->getWidth()*2*sizeof(int32)
            , &lightIndexListBufferID, _lightIndexListTBO->getWidth()*4*sizeof(int32));
    }
    else
    {
        _tileSpaceLightGrid->Update(_lightManager->GetFrustumAffectingLights(), _fpCamera, *_fpViewport
    ...
    // There's no interop available. So must do this manually
    if (_gpgpudevice==0)
    {
