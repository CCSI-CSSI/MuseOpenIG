...
LIBS += -losg -losgDB -losgSim -losgViewer -lOpenThreads -losgShadow -losgUtil\
...
#   Get the filename(only) list for distclean to remove only the files added from this plugin
    ...
    QMAKE_POST_LINK += test -d $$quote($$SDESTDIR) || $$QMAKE_MKDIR $$quote($$SDESTDIR) $$escape_expand(\\n\\t)
    QMAKE_POST_LINK += $$QMAKE_COPY $$quote(..\Resources\shaders\forwardplus*.glsl) $$quote($$SDESTDIR) $$escape_expand(\\n\\t)
    QMAKE_POST_LINK += $$QMAKE_COPY $$quote(..\Resources\shaders\shadow_*.glsl) $$quote($$SDESTDIR) $$escape_expand(\\n\\t)
    QMAKE_POST_LINK += $$QMAKE_COPY $$quote(..\Resources\shaders\lighting_math.glsl) $$quote($$SDESTDIR) $$escape_expand(\\n\\t)
}
