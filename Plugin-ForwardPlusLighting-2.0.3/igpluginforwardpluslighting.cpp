...
namespace OpenIG {
    namespace Plugins {
        ...
        public:
            ...
            {
            public:
                UpdateTODBasedLightingUniformCallback(bool& enabled, float& onDay, float& onNight, unsigned int& tod, OpenIG::Base::ImageGenerator* ig)
                    : _enabled(enabled)
                    , _onDay(onDay)
                    , _onNight(onNight)
                    , _tod(tod)
                ...
                virtual void operator () (osg::Uniform* u, osg::NodeVisitor*)
                {
                    if (_enabled)
                    {
                        float ambient = _ig->getSunOrMoonLight()->getLight()->getAmbient().g();
                        float factor = (1.f-ambient)*_onNight;
                        u->set(factor);
                    }
                    else
                    {
            ...
            std::string resourcesPath = OpenIG::Base::FileSystem::path(OpenIG::Base::FileSystem::Resources, "../../openig/resources");
                ...
                todBasedLightingUniform->setUpdateCallback(new UpdateTODBasedLightingUniformCallback(
                    _lightBrightness_enable, _lightBrightness_day, _lightBrightness_night, _todHour, context.getImageGenerator())
                    );
