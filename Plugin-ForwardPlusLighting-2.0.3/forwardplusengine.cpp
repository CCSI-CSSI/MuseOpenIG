...
ForwardPlusEngine::ForwardPlusEngine(
	...
	, _bUseMultipleCPUCores(bUseMultipleCPUCores)
{
...
osg::Matrix ForwardPlusEngine::computeWorldMatrix(osg::LightSource* lightSource)
{
...
bool ForwardPlusEngine::apply(osg::LightSource* lightSource, osgUtil::CullVisitor* cv)
{
	...
	unsigned int id = 0;
	if (!pLight->getUserValue("id", id)) return false;
	if (id == 0) return false;
			
	const osg::Matrixd& worldMatrix = computeWorldMatrix(lightSource);	
	const osg::Matrixd& viewMatrix = cv->getCurrentCamera()->getViewMatrix();	

	osg::Vec3d eye;
	osg::Vec3d ignore;
	cv->getCurrentCamera()->getViewMatrixAsLookAt(eye, ignore, ignore);
