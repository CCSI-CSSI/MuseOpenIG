...
ForwardPlusLightImplementationCallback::ForwardPlusLightImplementationCallback(OpenIG::Base::ImageGenerator* ig)
	...
	, _fpEngine(0)
{
	std::string strUseMultipleCPUCores = OpenIG::Base::Configuration::instance()->getConfig("UseMultipleCPUCores","no");
	bool bUseMultipleCPUCores = false;
	if (strUseMultipleCPUCores.compare(0, 3, "yes") == 0)
	{
		bUseMultipleCPUCores = true;
	}

	_lightManager = new LightManager(bUseMultipleCPUCores);

	_fpEngine = new ForwardPlusEngine(_ig, _ig->getScene()->asGroup(), *_lightManager, _fplights, bUseMultipleCPUCores);	

	std::string strUseGPUAcceleration = OpenIG::Base::Configuration::instance()->getConfig("UseGPUAcceleration","no");

	if (strUseGPUAcceleration.compare(0, 3, "yes") == 0)
	{
		_gpgpudevice = new GPGPUDevice(API_OPENGL);
		_gpgpudevice->Initialize();
		if (_gpgpudevice->Initialized()==false)
		{
			SAFE_DELETE(_gpgpudevice);
		}
	}
	else
	{
...
osg::Referenced* ForwardPlusLightImplementationCallback::createLight(unsigned int id, const OpenIG::Base::LightAttributes& definition, osg::Group* lightsGroup)
{
	...
	{		
		...
		if (_lightManagerStateAttribute.valid() == false)
		{
			...
			_lightManagerStateAttribute->set(_lightManager, _fpEngine->getFPCamera(), _fpEngine->getFPViewport(), _gpgpudevice, _fpEngine->useMultipleCores(), _ig->getScene()->asGroup(), _ig);
