...
void TBO::tearDown()
{
	if (_tboID!=0)
	{
		if (m_pGPGPUDevice)
		{
			m_pGPGPUDevice->UnregisterBuffer(&_tboID);
		}
...
void TBO::resize(size_t width)
{
	...
	if (m_pGPGPUDevice)
	{
		m_pGPGPUDevice->RegisterBuffer(&_tboID);
	}
}
...
TBO::TBO(size_t width, DataFormat format, const osg::GLExtensions* ext, GPGPUDevice* pDevice)
	: _ext(ext)
	, _isLocked(false)
	, _valid(true)
	, _tboID(0)
