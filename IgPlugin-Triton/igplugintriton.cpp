...
class TritonPlugin : public igplugincore::Plugin
{
public:
	TritonPlugin()
        : _geocentric(false)
	...
	class TritonCommand : public igcore::Commands::Command
	{
	public:
	...
	class TritonReflectionCommand : public igcore::Commands::Command
	{
	public:
    ...
    {
		igcore::Commands::instance()->addCommand("triton", new TritonCommand(this));
		igcore::Commands::instance()->addCommand("tritonreflection", new TritonReflectionCommand(this));
...
//            osg::notify(osg::ALWAYS)<<"WindowSystemInterface available creating windows."<<std::endl;
    ...
    osg::Camera* createHeightMapCamera(osg::Camera* mainCamera, osg::Node* model)
    {
        ...
        _heightMapcamera->setCullingActive(false);
    ...
    {        
        ...
        }
        {
            ...
            {
                Triton::WindFetch wf;
                wf.SetWind(attr->getValue()._speed, ((attr->getValue()._direction)*RAD_PER_DEG));
	...
	virtual void entityAdded(igplugincore::PluginContext& context, unsigned int entityID, osg::Node& node, const std::string& fileName)
	{
        if (entityID == 0)
        {
		...
		{
			osg::notify(osg::NOTICE) << "Triton: No XML found for " << fileName << std::endl;
			return;
		}
	...
	virtual void databaseRead(const std::string& fileName, osg::Node*, const osgDB::Options*)
	{
		...
		{
			osg::notify(osg::NOTICE) << "Triton: xml file does not exists: " << xmlFile << std::endl;
			return;
		}
		...
		if (!root)
		{
			osg::notify(osg::NOTICE) << "Triton: NULL root : " << xmlFile << std::endl;
			return;
		}
		if (!root->children.size())
		{
			osg::notify(osg::NOTICE) << "Triton: root with no children: " << xmlFile << std::endl;
			return;
		}
		if (root->children.at(0)->name != "OsgNodeSettings")
		{
			osg::notify(osg::NOTICE) << "Triton: OsgNodeSettings tag not found: " << xmlFile << std::endl;
			return;
		}

		osg::notify(osg::NOTICE) << "Triton: current file: " << fileName << std::endl;
...
protected:
    ...
    void initTriton(igcore::ImageGenerator* ig)
    {
        ...
        _tritonDrawable = new TritonDrawable(_path, _userName, _key, _geocentric);
...
{
    return new igplugins::TritonPlugin;
}
