...
class SetEntityNameCommand : public Commands::Command
{
public:
    SetEntityNameCommand(ImageGenerator* ig)
        : _ig(ig) {}
    ...
    virtual const std::string getDescription() const
    {
        return  "set the internal name of the entity id provided\n"
            "     id   - the id of the entity\n"
            "     name - the internal name to be set for the entity";
    }
...
public:
    ...
    virtual int exec(const StringUtils::Tokens& tokens)
    {
        if (tokens.size() == 2)
        {
            unsigned int id = atoi(tokens.at(0).c_str());
            bool enable = false;
            if(tokens.at(1) == "on" || tokens.at(1) == "true")
                enable = true;
        ...
        else if (tokens.size() == 3)
        {
            ...
            if(tokens.at(1) == "on" || tokens.at(1) == "true")
                enable = true;
            bool hard   = tokens.at(2) == "true";

            _ig->enableLight(id, enable, hard);
...
void Engine::initCommands()
{
    ...
    Commands::instance()->addCommand("setentityname", new SetEntityNameCommand(this));
