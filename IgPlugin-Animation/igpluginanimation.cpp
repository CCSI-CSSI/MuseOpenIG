...
class AnimationPlugin : public igplugincore::Plugin
{
public:
    ...
    virtual void update(igplugincore::PluginContext& context)
    {
        ...
        for ( ; itr != context.getAttributes().end(); ++itr)
        {
            ...
            {
                igcore::RefAnimationSequenceCallbacks* cbs = dynamic_cast<igcore::RefAnimationSequenceCallbacks*>(attr->getValue()._sequenceCallbacks.get());

                if (attr->getValue()._playback)
                {
                    igcore::Animations::instance()->playAnimation(
                        context.getImageGenerator(),
                        attr->getValue()._entityId,
                        attr->getValue()._animationName,
                        cbs
                    );
                }
                else
                {
                    if (attr->getValue()._reset)
                    {
                        igcore::Animations::instance()->resetAnimation(
                            context.getImageGenerator(),
                            attr->getValue()._entityId,
                            attr->getValue()._animationName
                        );
                    ...
                    {
                        igcore::Animations::instance()->stopAnimation(
                            context.getImageGenerator(),
                            attr->getValue()._entityId,
                            attr->getValue()._animationName
                        );
        ...
        igcore::Animations::instance()->updateAnimations(context.getImageGenerator());
    }
};
...
{
    return new igplugins::AnimationPlugin;
}
