...
TritonDrawable::TritonDrawable(const std::string& resourcePath, const std::string& userName, const std::string& licence, bool geocentric, bool forwardPlusEnabled, bool logZEnabled, osg::TextureCubeMap * _environmentMap)
...
void TritonDrawable::initializeLogZDepthBuffer(osg::RenderInfo& renderInfo, std::vector<GLint>& userShaders)
{
    ...
    {
        osg::notify(osg::NOTICE) << "Triton: Log Z Vertex Shader compiled successfully..." << std::endl;
    ...
    {
        osg::notify(osg::NOTICE) << "Triton: Log Z Pixel Shader compiled successfully..." << std::endl;
        userShaders.push_back(shaderID);
    }
}
...
void TritonDrawable::initializeForwardPlus(osg::RenderInfo& renderInfo, std::vector<GLint>& userShaders)
{
...

    {
        ...
        {
            osg::notify(osg::NOTICE) << "Triton:strForward_Plus_Triton_PS Shader compiled successfully..." << std::endl;
            userShaders.push_back(shaderID);
        }
    }
    ...
    if (_forwardPlusEnabled)
    {
        ...
        {
            osg::notify(osg::NOTICE) << "Triton: F+ Pixel Shader compiled successfully..." << std::endl;
            userShaders.push_back(shaderID);
        }
    }
}
