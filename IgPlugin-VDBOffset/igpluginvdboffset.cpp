...
public:
    ...
    {
        if (node.getName()=="db" && node.getNumParents())
        {
            _doOffset = false;
        }
...
class VDBOffsetPlugin : public igplugincore::Plugin
{
public:
    ...
    virtual void update(igplugincore::PluginContext& context)
    {
        ...
        if (attr)
        {
            ...
            igcore::StringUtils::Tokens tokensNewOffset = igcore::StringUtils::instance()->tokenize(newOffsetStr,",");
...
{
    return new igplugins::VDBOffsetPlugin;
}
